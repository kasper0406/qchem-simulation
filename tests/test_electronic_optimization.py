...
def test_permutation_equivariance_electronic_attention():
    ...
    distance_encoders = _create_distance_encoders(hdim, rngs)
    eatt = ElectronicAttention(hdim, num_electrons, num_nuclei,
                               rngs=rngs)
    ...
    original_out = eatt(dist, distance_encoders=distance_encoders)
    ...
    def _swap_and_check(i, j):
        ...
        swapped_out = eatt(swapped_dist, distance_encoders=distance_encoders)
...
def test_permutation_of_slater_determinant():
    ...
    distance_encoders = _create_distance_encoders(hdim, rngs) 
    slater_det = SlaterDeterminant(num_electrons, num_nuclei, hdim, rngs=rngs)
    ...
    dist = calculate_distances(electron_positions, nucleus_positions)
    spins = jnp.reshape((-1) ** jnp.arange(num_electrons), (num_electrons, 1))  # Alternating spins for testing
    original_det, original_sign, _ortho_measure = slater_det(dist, spins, distance_encoders=distance_encoders)
    ...
    def _swap_and_check(i, j):
        ...
        swapped_det, swapped_sign, _ortho_measure = slater_det(swapped_dist, swapped_spins, distance_encoders=distance_encoders)
